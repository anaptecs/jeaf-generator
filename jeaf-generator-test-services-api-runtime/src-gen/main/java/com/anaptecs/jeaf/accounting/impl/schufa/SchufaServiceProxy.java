...
/**
 * ServiceProxy class for JEAF service SchufaService.
 */
public final class SchufaServiceProxy extends ServiceProxy implements SchufaService {
  /**
   ...
   */
  public SchufaServiceProxy( TransactionBehavior pTransactionBehavior ) {
    super(SchufaService.class, pTransactionBehavior);
  }
  ...
  public boolean performSchufaCheck( SchufaRequest pRequest ) {
    try {
      Command lCommand = new PerformSchufaCheck_SchufaRequest_SchufaService_Command(pRequest);
...
/**
 * Generated command class for service method "performSchufaCheck".
 */
final class PerformSchufaCheck_SchufaRequest_SchufaService_Command extends Command {
  /**
  ...
  static {
    try {
      SERVICE_METHOD = SchufaService.class.getMethod("performSchufaCheck", SchufaRequest.class);
    }
    catch (NoSuchMethodException e) {
      throw new JEAFSystemException(MessageConstants.SERVICE_METHOD_DOES_NOT_EXIST, e, SchufaService.class.getName(),
          "performSchufaCheck(SchufaRequest.class)");
    }
  }

  /**
   * Attribute transports the method parameter "pRequest" to the service implementation via the service channel.
   */
  private final SchufaRequest request;

  /**
   * Initialize object. All parameters from method "performSchufaCheck" have to be passed as parameters to this command
   * object.
   *
   * @param pRequest SchufaRequest
   */
  PerformSchufaCheck_SchufaRequest_SchufaService_Command( SchufaRequest pRequest ) {
    super(SchufaService.class);
    request = pRequest;
  ...
  public Serializable execute( Service pTargetService ) {
    ...
    SchufaService lService = (SchufaService) pTargetService;
    // Trace service call.
    Trace lTrace = XFun.getTrace();
    lTrace.write(MessageConstants.EXECUTING_SERVICE_CALL, this.getCalledServiceMethod());
    long lStartTime = System.nanoTime();
    Serializable lResult = (Serializable) lService.performSchufaCheck(request);
