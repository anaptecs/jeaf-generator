...
public class CurrencyCodeDeserializer extends JsonDeserializer<CurrencyCode> {
  ...
  public CurrencyCode deserialize( JsonParser pParser, DeserializationContext pContext ) throws IOException {
    ...
    if (lNode instanceof TextNode) {
      return CurrencyCode.Builder.newBuilder().setCode(lNode.asText()).build();
    }
    // Node is not a TextNode
    else {
      throw new IOException(
          "Unable to deserialize object of type CurrencyCode. Expected text node but received other type of JSON content. Current JSON node ''"
