...
public class ByteCodeTypeDeserializer extends JsonDeserializer<ByteCodeType> {
  ...
  public ByteCodeType deserialize( JsonParser pParser, DeserializationContext pContext ) throws IOException {
    ...
    if (lNode instanceof TextNode) {
      return ByteCodeType.Builder.newBuilder().setCode((byte) lNode.asInt()).build();
    }
    // Node is not a TextNode
    else {
      throw new IOException(
          "Unable to deserialize object of type ByteCodeType. Expected text node but received other type of JSON content. Current JSON node ''"
