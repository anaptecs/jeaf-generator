...
public class IntegerCodeTypeDeserializer extends JsonDeserializer<IntegerCodeType> {
  ...
  public IntegerCodeType deserialize( JsonParser pParser, DeserializationContext pContext ) throws IOException {
    ...
    if (lNode instanceof TextNode) {
      return IntegerCodeType.Builder.newBuilder().setCode(lNode.asInt()).build();
    }
    // Node is not a TextNode
    else {
      throw new IOException(
          "Unable to deserialize object of type IntegerCodeType. Expected text node but received other type of JSON content. Current JSON node ''"
