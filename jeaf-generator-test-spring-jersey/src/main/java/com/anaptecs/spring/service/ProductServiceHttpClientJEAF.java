...
public class ProductServiceHttpClientJEAF {
  ...
  public <T> T executeCollectionResultRequest( ClassicHttpRequest pRequest, int pSuccessfulStatusCode,
    ...
    return executeRequest(pRequest, pSuccessfulStatusCode, ObjectType.createObjectType(pCollectionClass, pTypeClass));
  }

  public <T> T executeSingleObjectResultRequest( ClassicHttpRequest pRequest, int pSuccessfulStatusCode,
      Class<?> pTypeClass ) {
    return executeRequest(pRequest, pSuccessfulStatusCode, ObjectType.createObjectType(pTypeClass));
  }

  private <T> T executeRequest( ClassicHttpRequest pRequest, int pSuccessfulStatusCode, ObjectType pResponseType ) {
    ...
    try {
      ...
      if (lStatusCode == pSuccessfulStatusCode) {
        return JSON.getJSONTools().read(lResponse.getEntity().getContent(), pResponseType);
      }
    ...
    // Thanks to resilience4j circuit breaker interface definition we also have to catch java.lang.Exception ;-(
    catch (Exception e) {
      throw new JEAFSystemException(JSONMessages.REST_RESPONSE_PROCESSING_ERROR, e, pRequest.getRequestUri(),
          e.getMessage());
    }
