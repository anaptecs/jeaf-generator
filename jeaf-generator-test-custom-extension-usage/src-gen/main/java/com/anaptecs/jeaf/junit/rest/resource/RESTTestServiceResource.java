...
public class RESTTestServiceResource {
  ...
  @GET
  @JEAFCustomAnnotationTest
  public Response getResource( @PathParam("{ID}") @JEAFCustomAnnotationTest String pID ) {
  ...
  public void updateTestServiceObject( @Suspended AsyncResponse pAsyncResponse,
      @javax.ws.rs.core.Context HttpServletRequest pRequest, @JEAFCustomAnnotationTest TestServiceObject pObject ) {
  ...
  @JEAFCustomAnnotationTest
  public Response yetAnotherRESTOperation( @PathParam("id") @JEAFCustomAnnotationTest String pID,
      @HeaderParam("token") @JEAFCustomAnnotationTest String pToken,
      @QueryParam("filter") @JEAFCustomAnnotationTest String pQueryParam1,
      @CookieParam("cookieEntry") @JEAFCustomAnnotationTest String pCookieParam ) {
    // Delegate request to service.
    RESTTestService lService = this.getRESTTestService();
    boolean lResult = lService.yetAnotherRESTOperation(pID, pToken, pQueryParam1, pCookieParam);
    return Response.status(Response.Status.CREATED).entity(lResult).build();
  }
  ...
  @GET
  @JEAFCustomAnnotationTest
  public Response handleBeanParam1( @BeanParam @JEAFCustomAnnotationTest Context pContext,
  ...
  @POST
  @JEAFCustomAnnotationTest
  public Response handleBeanParam1( @BeanParam @JEAFCustomAnnotationTest BeanParameter pBeanParam ) {
    // Delegate request to service.
    RESTTestService lService = this.getRESTTestService();
  ...
  @GET
  @JEAFCustomAnnotationTest
  public Response deprecatedParameter( @HeaderParam("pParam1") @JEAFCustomAnnotationTest int pParam1,
      @HeaderParam("pParam2") @Deprecated @JEAFCustomAnnotationTest int pParam2 ) {
    // Delegate request to service.
    RESTTestService lService = this.getRESTTestService();
  ...
  @JEAFCustomAnnotationTest
  @Deprecated
  public Response deprectaedReturnValue( @HeaderParam("pParam1") @JEAFCustomAnnotationTest int pParam1 ) {
    // Delegate request to service.
    RESTTestService lService = this.getRESTTestService();
  ...
  public void deprecatedAsyncParam( @Suspended AsyncResponse pAsyncResponse,
      @javax.ws.rs.core.Context HttpServletRequest pRequest, @Deprecated @JEAFCustomAnnotationTest int pParam1 ) {
    // Lookup workload manager that takes care that the system will have an optimal throughput.
    WorkloadManager lWorkloadManager = Workload.getWorkloadManager();
    // Prepare meta information about the request.
    String lEndpointURL = pRequest.getServletPath() + pRequest.getPathInfo();
    ...
    lWorkloadManager.execute(lRequestInfo, new RESTWorkloadErrorHandler(pAsyncResponse), new Runnable() {
      ...
      public void run( ) {
        try {
          ...
          Response lResponseObject = Response.status(Response.Status.CREATED).entity(lResult).build();
          // Due to the asynchronous processing of the requests, the response can not be returned as return value.
          // Therefore we make use of the defined JAX-RS mechanisms.
          pAsyncResponse.resume(lResponseObject);
        }
  ...
  public void deprecatedAsync( @Suspended AsyncResponse pAsyncResponse,
      @javax.ws.rs.core.Context HttpServletRequest pRequest, @JEAFCustomAnnotationTest int pParam1 ) {
    // Lookup workload manager that takes care that the system will have an optimal throughput.
    WorkloadManager lWorkloadManager = Workload.getWorkloadManager();
    // Prepare meta information about the request.
    String lEndpointURL = pRequest.getServletPath() + pRequest.getPathInfo();
