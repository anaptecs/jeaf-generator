...
public class BooleanCodeTypeDeserializer extends JsonDeserializer<BooleanCodeType> {
  ...
  public BooleanCodeType deserialize( JsonParser pParser, DeserializationContext pContext ) throws IOException {
    ...
    if (lNode instanceof TextNode) {
      return BooleanCodeType.Builder.newBuilder().setCode(lNode.asBoolean()).build();
    }
    // Node is not a TextNode
    else {
      throw new IOException(
          "Unable to deserialize object of type BooleanCodeType. Expected text node but received other type of JSON content. Current JSON node ''"
