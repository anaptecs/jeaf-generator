 ...
 */
public class FloatCodeTypeDeserializerV3 extends ValueDeserializer<FloatCodeType> {
  @Override
  public FloatCodeType deserialize( JsonParser pParser, DeserializationContext pContext ) {
    // Parse JSON content.
    ...
    if (lNode instanceof NumericNode) {
      ...
      return FloatCodeType.of((float) lNumericNode.doubleValue());
    }
    // Node is not of expected type.
    else {
      throw DatabindException.from(pContext,
          "Unable to deserialize object of type FloatCodeType. Expected node type that matches to data type but received other JSON content. Current JSON node ''"
