 ...
 */
public class StringCodeDeserializerV3 extends ValueDeserializer<StringCode> {
  @Override
  public StringCode deserialize( JsonParser pParser, DeserializationContext pContext ) {
    ...
    if (lNode instanceof StringNode) {
      return StringCode.of(lNode.asString());
    }
    // Node is not of expected type.
    else {
      throw DatabindException.from(pContext,
          "Unable to deserialize object of type StringCode. Expected node type that matches to data type but received other JSON content. Current JSON node ''"
