...
/**
 * Class implements an bidirectional object mapper between classes {@link POJOWithID} as source and
 * {@link VerbindungspunktDO} as target.
 */
public class POJOWithIDToVerbindungspunktDOConverter implements ObjectMapper<POJOWithID, VerbindungspunktDO> {
  /**
   * Only instance of this class. We can avoid to create multiple instances as object mapper implementations are
   * stateless and thread safe.
   */
  private static final POJOWithIDToVerbindungspunktDOConverter INSTANCE = new POJOWithIDToVerbindungspunktDOConverter();

  /**
   * Constructor is private. In order to avoid that unnecessary instances are created.
   */
  private POJOWithIDToVerbindungspunktDOConverter( ) {
  }

  /**
   * Method returns an instance of this object mapper.
   *
   * @return {@link ObjectMapper} Instance an object mapper that is capable to map between POJOWithID and
   * VerbindungspunktDO. The method never returns null.
   */
  public static POJOWithIDToVerbindungspunktDOConverter getConverter( ) {
    return INSTANCE;
  }

  /**
   * Method creates a new VerbindungspunktDO object from the passed POJOWithID object and applies the defined mappings
   * of type {@link MappingType#CREATE} from the UML model.
   *
   * @param pSource POJOWithID object that should be used to create a new VerbindungspunktDO object. The parameter must
   * not be null.
   * @return {@link VerbindungspunktDO} Created VerbindungspunktDO object. The method never returns null.
   */
  public static VerbindungspunktDO createNewTargetObject( POJOWithID pSource ) {
    // Check parameter
  ...
  /**
   * Method creates new VerbindungspunktDO objects based on the passed POJOWithID objects. For every POJOWithID object a
   * new VerbindungspunktDO object will be returned. The mappings for case {@link MappingType#CREATE} will be applied.
   *
   * @param pSourceObjects All POJOWithID objects that should be used to create VerbindungspunktDO objects. The
   * parameter must not be null.
   * @return {@link List} List with all VerbindungspunktDO objects that were created. The method never returns null.
   */
  public static List<VerbindungspunktDO> createNewTargetObjects( Collection<POJOWithID> pSourceObjects ) {
    // Check parameter
    ...
    for (POJOWithID lNext : pSourceObjects) {
      VerbindungspunktDO lNewTargetObject = POJOWithIDToVerbindungspunktDOConverter.createNewTargetObject(lNext);
   ...
   */
  @Override
  public void applyMappings( POJOWithID pSource, VerbindungspunktDO pTarget, ObjectMapper.MappingType pMappingType ) {
    // Check parameters
    Check.checkInvalidParameterNull(pSource, "pSource");
   ...
   */
  @Override
  public void applyMappings( List<POJOWithID> pSources, List<VerbindungspunktDO> pTargets,
   ...
   * UML model.
   *
   * @param pTarget VerbindungspunktDO object that should be used to create a new POJOWithID object. The parameter must
   * not be null.
   * @return {@link POJOWithID} Created POJOWithID object. The method never returns null.
   */
  public static POJOWithID createNewSourceObject( VerbindungspunktDO pTarget ) {
    // Check parameter
    Check.checkInvalidParameterNull(pTarget, "pTarget");
  ...
  /**
   * Method creates new POJOWithID objects based on the passed VerbindungspunktDO objects. For every VerbindungspunktDO
   * object a new POJOWithID object will be returned. The mappings for case {@link MappingType#READ} will be applied.
   *
   ...
   */
  public static List<POJOWithID> createNewSourceObjects( Collection<VerbindungspunktDO> pTargetObjects ) {
    // Check parameter
    ...
    for (VerbindungspunktDO lNext : pTargetObjects) {
      POJOWithID lNewSourceObject = POJOWithIDToVerbindungspunktDOConverter.createNewSourceObject(lNext);
  ...
  /**
   * Method maps all attributes from POJOWithID to VerbindungspunktDO. Mappings are applied only for case
   ...
   */
  private void setTargetObject( POJOWithID pSource, VerbindungspunktDO pTarget ) {
    pTarget.setBavName(pSource.getName());
  }
   ...
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void updateTargetObject( POJOWithID pSource, VerbindungspunktDO pTarget ) {
    // Update target object
    pTarget.setBavName(pSource.getName());
  }

  /**
   * Method maps all attributes from VerbindungspunktDO to POJOWithID. Mappings are applied only for case
   * {@link MappingType#READ}.
   *
   * @param pSource Source object for the mapping. The parameter must not be null.
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void setSourceObject( POJOWithID pSource, VerbindungspunktDO pTarget ) {
    pSource.setName(pTarget.getBavName());
  }
}
