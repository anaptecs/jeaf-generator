...
/**
 * Class implements an bidirectional object mapper between classes {@link ClassA} as source and {@link ClassA_BO} as
 * target.
 */
public class ClassAToClassA_BOConverter implements ObjectMapper<ClassA, ClassA_BO> {
  /**
   * Only instance of this class. We can avoid to create multiple instances as object mapper implementations are
   * stateless and thread safe.
   */
  private static final ClassAToClassA_BOConverter INSTANCE = new ClassAToClassA_BOConverter();

  /**
   * Constructor is private. In order to avoid that unnecessary instances are created.
   */
  private ClassAToClassA_BOConverter( ) {
  }

  /**
   * Method returns an instance of this object mapper.
   * 
   * @return {@link ObjectMapper} Instance an object mapper that is capable to map between ClassA and ClassA_BO. The
   * method never returns null.
   */
  public static ClassAToClassA_BOConverter getConverter( ) {
    return INSTANCE;
  }

  /**
   * Method creates a new ClassA_BO object from the passed ClassA object and applies the defined mappings of type
   * {@link MappingType#CREATE} from the UML model.
   * 
   * @param pSource ClassA object that should be used to create a new ClassA_BO object. The parameter must not be null.
   * @return {@link ClassA_BO} Created ClassA_BO object. The method never returns null.
   */
  public static ClassA_BO createNewTargetObject( ClassA pSource ) {
    // Check parameter
  ...
  /**
   * Method creates new ClassA_BO objects based on the passed ClassA objects. For every ClassA object a new ClassA_BO
   * object will be returned. The mappings for case {@link MappingType#CREATE} will be applied.
   * 
   * @param pSourceObjects All ClassA objects that should be used to create ClassA_BO objects. The parameter must not be
   * null.
   * @return {@link List} List with all ClassA_BO objects that were created. The method never returns null.
   */
  public static List<ClassA_BO> createNewTargetObjects( Collection<ClassA> pSourceObjects ) {
    // Check parameter
    ...
    for (ClassA lNext : pSourceObjects) {
      ClassA_BO lNewTargetObject = ClassAToClassA_BOConverter.createNewTargetObject(lNext);
   ...
   * 
   * @param pSource ClassA object that should be used to update an existing ClassA_BO object. The parameter must not be
   * null.
   * @return {@link ClassA_BO} Updated ClassA_BO object. The method never returns null.
   */
  public static ClassA_BO updatePersistentTarget( ClassA pSource ) {
    // Check parameter
    Check.checkInvalidParameterNull(pSource, "pSource");
   ...
   */
  @Override
  public void applyMappings( ClassA pSource, ClassA_BO pTarget, ObjectMapper.MappingType pMappingType ) {
    // Check parameters
    Check.checkInvalidParameterNull(pSource, "pSource");
   ...
   */
  @Override
  public void applyMappings( List<ClassA> pSources, List<ClassA_BO> pTargets, ObjectMapper.MappingType pMappingType ) {
    // Check parameters
   ...
   * model.
   * 
   * @param pTarget ClassA_BO object that should be used to create a new ClassA object. The parameter must not be null.
   * @return {@link ClassA} Created ClassA object. The method never returns null.
   */
  public static ClassA createNewSourceObject( ClassA_BO pTarget ) {
    // Check parameter
    Check.checkInvalidParameterNull(pTarget, "pTarget");
  ...
  /**
   * Method creates new ClassA objects based on the passed ClassA_BO objects. For every ClassA_BO object a new ClassA
   ...
   */
  public static List<ClassA> createNewSourceObjects( Collection<ClassA_BO> pTargetObjects ) {
    // Check parameter
    ...
    for (ClassA_BO lNext : pTargetObjects) {
      ClassA lNewSourceObject = ClassAToClassA_BOConverter.createNewSourceObject(lNext);
  ...
  /**
   * Method maps all attributes from ClassA to ClassA_BO. Mappings are applied only for case {@link MappingType#READ}.
   * 
   ...
   */
  private void setTargetObject( ClassA pSource, ClassA_BO pTarget ) {
    pTarget.setName(pSource.getName());
  }

  /**
   * Method maps all attributes from ClassA to ClassA_BO. Mappings are applied only for case {@link MappingType#UPDATE}.
   * 
   * @param pSource Source object for the mapping. The parameter must not be null.
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void updateTargetObject( ClassA pSource, ClassA_BO pTarget ) {
  ...
  /**
   * Method maps all attributes from ClassA_BO to ClassA. Mappings are applied only for case {@link MappingType#READ}.
   * 
   * @param pSource Source object for the mapping. The parameter must not be null.
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void setSourceObject( ClassA pSource, ClassA_BO pTarget ) {
