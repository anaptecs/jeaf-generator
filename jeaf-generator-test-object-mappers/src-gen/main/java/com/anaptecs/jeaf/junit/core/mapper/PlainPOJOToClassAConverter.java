...
/**
 * Class implements an bidirectional object mapper between classes {@link PlainPOJO} as source and {@link ClassA} as
 * target.
 */
public class PlainPOJOToClassAConverter implements ObjectMapper<PlainPOJO, ClassA> {
  /**
   * Only instance of this class. We can avoid to create multiple instances as object mapper implementations are
   * stateless and thread safe.
   */
  private static final PlainPOJOToClassAConverter INSTANCE = new PlainPOJOToClassAConverter();

  /**
   * Constructor is private. In order to avoid that unnecessary instances are created.
   */
  private PlainPOJOToClassAConverter( ) {
  }

  /**
   * Method returns an instance of this object mapper.
   * 
   * @return {@link ObjectMapper} Instance an object mapper that is capable to map between PlainPOJO and ClassA. The
   * method never returns null.
   */
  public static PlainPOJOToClassAConverter getConverter( ) {
    return INSTANCE;
  }

  /**
   * Method creates a new ClassA object from the passed PlainPOJO object and applies the defined mappings of type
   * {@link MappingType#CREATE} from the UML model.
   * 
   * @param pSource PlainPOJO object that should be used to create a new ClassA object. The parameter must not be null.
   * @return {@link ClassA} Created ClassA object. The method never returns null.
   */
  public static ClassA createNewTargetObject( PlainPOJO pSource ) {
    // Check parameter
    Check.checkInvalidParameterNull(pSource, "pSource");
    // Create new target object.
    ClassA lTarget = ClassA.Builder.newBuilder().build();
    PlainPOJOToClassAConverter lObjectMapper = PlainPOJOToClassAConverter.getConverter();
  ...
  /**
   * Method creates new ClassA objects based on the passed PlainPOJO objects. For every PlainPOJO object a new ClassA
   * object will be returned. The mappings for case {@link MappingType#CREATE} will be applied.
   * 
   * @param pSourceObjects All PlainPOJO objects that should be used to create ClassA objects. The parameter must not be
   * null.
   * @return {@link List} List with all ClassA objects that were created. The method never returns null.
   */
  public static List<ClassA> createNewTargetObjects( Collection<PlainPOJO> pSourceObjects ) {
    // Check parameter
    ...
    for (PlainPOJO lNext : pSourceObjects) {
      ClassA lNewTargetObject = PlainPOJOToClassAConverter.createNewTargetObject(lNext);
   ...
   */
  @Override
  public void applyMappings( PlainPOJO pSource, ClassA pTarget, ObjectMapper.MappingType pMappingType ) {
    // Check parameters
    Check.checkInvalidParameterNull(pSource, "pSource");
   ...
   */
  @Override
  public void applyMappings( List<PlainPOJO> pSources, List<ClassA> pTargets, ObjectMapper.MappingType pMappingType ) {
    // Check parameters
   ...
   * UML model.
   * 
   * @param pTarget ClassA object that should be used to create a new PlainPOJO object. The parameter must not be null.
   * @return {@link PlainPOJO} Created PlainPOJO object. The method never returns null.
   */
  public static PlainPOJO createNewSourceObject( ClassA pTarget ) {
    // Check parameter
    Check.checkInvalidParameterNull(pTarget, "pTarget");
  ...
  /**
   * Method creates new PlainPOJO objects based on the passed ClassA objects. For every ClassA object a new PlainPOJO
   ...
   */
  public static List<PlainPOJO> createNewSourceObjects( Collection<ClassA> pTargetObjects ) {
    // Check parameter
    ...
    for (ClassA lNext : pTargetObjects) {
      PlainPOJO lNewSourceObject = PlainPOJOToClassAConverter.createNewSourceObject(lNext);
  ...
  /**
   * Method maps all attributes from PlainPOJO to ClassA. Mappings are applied only for case {@link MappingType#READ}.
   * 
   ...
   */
  private void setTargetObject( PlainPOJO pSource, ClassA pTarget ) {
    pTarget.setName(pSource.getHello());
  }

  /**
   * Method maps all attributes from PlainPOJO to ClassA. Mappings are applied only for case {@link MappingType#UPDATE}.
   * 
   * @param pSource Source object for the mapping. The parameter must not be null.
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void updateTargetObject( PlainPOJO pSource, ClassA pTarget ) {
    // Update target object
    pTarget.setName(pSource.getHello());
  }

  /**
   * Method maps all attributes from ClassA to PlainPOJO. Mappings are applied only for case {@link MappingType#READ}.
   * 
   * @param pSource Source object for the mapping. The parameter must not be null.
   * @param pTarget Target object for the mapping. The parameter must not be null.
   */
  private void setSourceObject( PlainPOJO pSource, ClassA pTarget ) {
    pSource.setHello(pTarget.getName());
  }
}
