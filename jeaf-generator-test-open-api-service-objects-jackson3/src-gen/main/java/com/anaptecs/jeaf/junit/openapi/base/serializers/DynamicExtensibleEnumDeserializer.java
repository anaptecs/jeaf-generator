 ...
 */
public class DynamicExtensibleEnumDeserializer extends ValueDeserializer<DynamicExtensibleEnum> {
  @Override
  public DynamicExtensibleEnum deserialize( JsonParser pParser, DeserializationContext pContext ) {
    ...
    if (lNode instanceof StringNode) {
      ...
      return DynamicExtensibleEnum.valueOf(lLiteralName);
    }
    // Node is not a TextNode
    else {
      throw new JEAFSystemException(JSONMessages.UNEXPECTED_NODE_TYPE_FOR_DESERIALIZATION,
          DynamicExtensibleEnum.class.getName(), lNode.toString(), lNode.getClass().getName());
    }
  }
}